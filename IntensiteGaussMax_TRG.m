function [I_exp,sig_I_exp,Fit] = IntensiteGaussMax_TRG(E,Lambda,Int,Doublet,GraphAll,GraphExp)
%% ========================================================================================
%% ========== l'intensité des raies est extraite en fittant une/des gaussienne =========
%% ========================================================================================
%% ====== INFOS SUR LA FONCTION ======

% E (vecteur) contient les longueurs d'onde des raies à fitter (nm)

% Lambda (vecteur) les longueur du spectre en nm.

% Int (vecteur) intensité associé à un lambda donnée

% Doublet est un vecteur de valeurs (0, 1 ou 2) associé à la raie à fitter
    % (E) qui demande à deconvoluer 1, 2 ou 3 gausiennes. Si vous avez un
    % l'intensité de la raie demandé (E) est seulement extraite par contre.
    
% GraphAll est une variable binaire (0 ou 1) qui affiche tout les fits sur
    %le spectre
    
% GraphExp est une variable bianire (0 ou 1) qui affiche chaque fit
    % individuellement.
%------------------------------------------------------------------------------------
%I_exp = vecteur avec les intensités des raies fitté
%sig_I_exp= incertitude sur les fits
%Fit = vector of boul that gives if the function has fit or not the line
...
if GraphAll==1
    ...
    scatter(E,temp_I_exp,'MarkerEdgeColor',[1 0.5 0.2])
